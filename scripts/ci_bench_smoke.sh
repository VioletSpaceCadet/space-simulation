...
# Progression smoke (validates progression_start.json doesn't deadlock)
echo "  running progression_ci_smoke scenario..."
"$BENCH" run --scenario "$REPO_ROOT/scenarios/progression_ci_smoke.json" --output-dir "$OUTPUT_DIR"

PROG_RUN_DIR=$(find "$OUTPUT_DIR" -maxdepth 1 -type d -name 'progression_ci_smoke_*' | sort | tail -1)

if [ -z "$PROG_RUN_DIR" ] || [ ! -f "$PROG_RUN_DIR/batch_summary.json" ]; then
  echo "ERROR: batch_summary.json not found in $OUTPUT_DIR/progression_ci_smoke_*/"
  exit 1
fi
