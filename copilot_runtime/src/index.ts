...
function main(): void {
  ...
  // our own `cors()` first.
  //
  // Multi-route mode is the v2 default. The `@copilotkit/react-core/v2`
  // client auto-detects transport via a GET probe to `basePath/info`;
  // when that succeeds it uses multi-route (`POST basePath/agent/{id}/run`)
  // for agent calls. Single-route mode coalesces multiple agent turns into
  // a single message bubble on the client — a CopilotKit 1.55.3 quirk we
  // observed during Mb2 manual testing. Multi-route renders each turn as
  // its own assistant bubble.
