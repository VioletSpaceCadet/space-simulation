...
use sim_core::{
    AsteroidTemplateDef, Constants, Counters, EventLevel, FacilitiesState, GameContent,
    GameState, MetaState, NodeId, PrincipalId, ResearchState, ShipId, ShipState, SiteId,
    ScanSite, SolarSystemDef, StationId, StationState, TechDef,
};
...
fn load_content(content_dir: &str) -> Result<GameContent> {
    ...
    let constants: Constants = serde_json::from_str(
        &std::fs::read_to_string(dir.join("constants.json"))
            .context("reading constants.json")?,
    )
...
fn build_initial_state(content: &GameContent, seed: u64, rng: &mut impl rand::Rng) -> GameState {
    ...
    let node_ids: Vec<&NodeId> =
        content.solar_system.nodes.iter().map(|n| &n.id).collect();
...
fn run(
    ...
    for _ in 0..ticks {
        let commands =
            autopilot.generate_commands(&state, &content, &mut next_command_id);
        ...
        for event in &events {
            if let sim_core::Event::TechUnlocked { tech_id } = &event.event {
                println!("*** TECH UNLOCKED: {tech_id} at tick={:04} ***", state.meta.tick);
            }
        }
...
fn print_status(state: &GameState) {
    ...
    let unlocked: Vec<String> =
        state.research.unlocked.iter().map(|t| t.0.clone()).collect();
...
fn main() -> Result<()> {
    let cli = Cli::parse();
    match cli.command {
        Commands::Run { ticks, seed, content_dir, print_every, event_level } => {
