    ...
    for ship_id in ship_ids {
        ...
        match task_kind {
            TaskKind::Transit { ref destination, ref then, .. } => {
                tasks::resolve_transit(state, &ship_id, destination, then, content, events);
            }
            TaskKind::Survey { ref site } => {
                tasks::resolve_survey(state, &ship_id, site, content, rng, events);
            }
            TaskKind::DeepScan { ref asteroid } => {
                tasks::resolve_deep_scan(state, &ship_id, asteroid, content, rng, events);
            }
            TaskKind::Idle => {}
        }
    }
