...
date: 2026-02-24
module: sim_world
component: build_initial_state, dev_base_state.json
...
The gameplay loop was completely deadlocked. Neither `dev_base_state.json` nor `build_initial_state()` included labs, which meant:
...
Additionally, `build_initial_state()` (programmatic) and `dev_base_state.json` (authored JSON) had diverged. The programmatic version only had a maintenance bay and 5 repair kits, while the JSON version had a refinery, assembler, and more starting materials.

## Solution

1. **Added labs to dev_base_state.json** (VIO-5): Added `module_exploration_lab` and `module_materials_lab` to starting station inventory, enabling the research → tech unlock → deep scan → mining chain.
...
- Keep `build_initial_state()` and `dev_base_state.json` in sync. If you update one, update the other.
