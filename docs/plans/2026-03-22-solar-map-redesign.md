# Solar System Map Redesign

**Goal:** Replace SVG-based map with canvas+DOM overlay for LOD-based rendering, rich tooltips, keyboard navigation, and seamless zoom across the solar system.
**Status:** Planned
**Linear Project:** [Solar System Map Redesign](https://linear.app/violetspacecadet/project/solar-system-map-redesign-9bb20978caf9)

**Scope:** Phases 1-4 (Sol system canvas refresh). Phase 5 (multi-system) deferred until game content requires it.
**Mockup:** `docs/plans/solar-map-mockup.html`

## Overview

Interactive canvas-based map replacing the current SVG-based `SolarSystemMap.tsx`. Supports seamless zoom, LOD-based decluttering, rich tooltips, keyboard navigation, and transit line rendering. Pure FE project — no sim_core or daemon changes needed.
...
- **Mockup:** `docs/plans/solar-map-mockup.html` (~800 lines, single-file HTML/CSS/JS)
...
### Solar System Map Redesign (Phases 1-4)

1. **MAP-01: Canvas map core** — New SolarSystemMapCanvas.tsx, canvas rendering with requestAnimationFrame, camera system (zoom/pan/worldToScreen/screenToWorld), starfield CSS background tile, render all entity types (bodies, zones, orbit rings, stations, ships, asteroids, scan sites). Consumes same props as current SVG map.
2. **MAP-02: LOD + interaction** — LOD tier system (Interstellar/System/Region/Local) with smoothStep fading, zoom-to-cursor with log-space lerp, drag-to-pan with zoom-scaled speed, hover detection with HTML tooltip overlay, click-to-select with DetailCard, double-click zoom-to-entity.
3. **MAP-03: Navigation HUD** — Quick-nav panel (HTML overlay, context-aware waypoints), keyboard shortcuts (1-9 for locations, Shift for stations, Ctrl for ships), breadcrumb navigation, minimap with click-to-pan.
4. **MAP-04: Transit rendering + rich tooltips** — Transit line rendering with dashed line, progress pip, ETA label, destination marker. Rich HTML tooltips for all entity types (station, ship, asteroid, scan site, body). Ship task color coding from config/theme.ts. Asteroid irregular polygon shapes.
5. **MAP-05: Cleanup + d3 removal** — Audit d3-selection/d3-zoom usage, remove if map-only. Remove old SolarSystemMap.tsx and useSvgZoomPan.ts. Update imports throughout.
6. **MAP-06: Testing** — vitest for canvas utilities (coordinate transforms, LOD tier computation, camera math), chrome-based visual testing for rendering correctness and interaction.

Dependencies: MAP-01 → MAP-02 → MAP-03; MAP-01 → MAP-04; MAP-01 → MAP-05; all → MAP-06
